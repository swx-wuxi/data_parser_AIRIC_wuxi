...
# === 轨迹配置参数 ===
SCENE_DURATION = 20  # 场景固定时长20秒
FRAME_TIMES = [3, 11]  # 需要采样的帧时间点（秒）
HISTORY_DURATION = 2.0  # 历史轨迹时长
FUTURE_DURATION = 8.0   # 未来轨迹时长
TIME_TOLERANCE = 0.5 * 1e9  # 轨迹时间对齐容差
OB_TIME_TOLERANCE = 0.5 * 1e9  # object时间对齐容差
MAX_NEIGHBORS = 20      # 最大邻居数量

# === 地图配置参数 ===
MAP_PATH = "/home/WorkSpace/map/base_map.bin"
...
def process_vector_map(ego_x, ego_y, ego_heading, 
    ...
    def find_current_lane(hd_map, ego_x, ego_y):
        ...
        for lane in hd_map.lane:
            ...
            if not (x_min-5 <= ego_x <= x_max+5 and y_min-5 <= ego_y <= y_max+5):
                continue
            # 精确计算距离
            center_line = [[p.x, p.y] for p in lane.central_curve.segment[0].line_segment.point]
    ...
    def calculate_heading_change(current_lane, successor_lane):
        ...
        try:
            ...
            successor_heading = math.atan2(dy_s, dx_s)
            # 计算航向变化（规范化到[-pi, pi]）
...
def process_neighbor_trajectory(obj, current_obs_time, history_timestamps, future_timestamps, 
    ...
    def get_ego_state(t_ns):
        ...
        return (pose.position_enu.x, pose.position_enu.y, pose.heading)
    ...
    for t in history_timestamps:
        try:
            ...
            if obs_idx is not None and obs_idx < len(obs_list):
                found = next((o for o in obs_list[obs_idx][1].perception_obstacles 
                            if o.id == obj.id), None)
    ...
    for scene_idx, (scene_start, scene_end) in enumerate(scene_windows):
        ...
        for frame_offset in FRAME_TIMES:
            ...
            if len(current_obs) < 3 or current_obs[0] != 'obs':
                ...
                continue
            if not hasattr(current_obs[1], 'perception_obstacles'):
                logging.info("current_obs message missing perception_obstacles")
                continue
            ...
            # 坐标系参数
            ego_x = current_pose.position_enu.x
            ego_y = current_pose.position_enu.y
            ego_heading = current_pose.heading

            # === 1. 自车历史轨迹 ===
            logging.info("采样自车历史轨迹")
            ...
            if not future_samples or len(future_samples) < 80:
                logging.info("未来采样轨迹数据不足80个点，跳过该帧")
                continue
            ...
            # === 3. 邻居车辆处理 ===
            #logging.info("开始感知车辆处理")
            current_obstacles = {obj.id: obj for obj in current_obs[1].perception_obstacles}
            neighbors = []
        ...
        if len(parts) < 8:
            ...
            continue
        task_folder = parts[4]  # 获取任务文件夹名称
        timestamp = parts[7]  # 获取时间戳部分
        # 保存场景数据
